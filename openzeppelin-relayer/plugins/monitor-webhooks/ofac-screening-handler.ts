 ...
 */
export async function handler(api: PluginAPI, params: OFACScreeningParams): Promise<any> {
    ...
    const sanctionedRecipients = recipients.filter(addr => 
        OFAC_SANCTIONED_ADDRESSES.map(a => a.toLowerCase())
